...
//cv::Mat		features( const cv::Mat &image, const int sbin );
...
namespace PM_type{
	...
	cv::Mat		features( const cv::Mat &image, const int sbin, const int pad[2] )
	{
		...
		float		*hist[18];	
		int			hist_step[18];
		cv::Mat		hist_tmp( blocks[0], blocks[1]*18, CV_32FC1, cv::Scalar(0) );
		for( int i=0; i<18; i++ ){
			//histmat[i] = cv::Mat::zeros( blocks[0], blocks[1], CV_32FC1 );
		...
		cv::Mat		normmat( blocks[0], blocks[1], CV_32FC1, cv::Scalar(0) );
		float * const norm = normmat.ptr<float>(0,0);
		const int	norm_step = normmat.step1();

		cv::Mat	histmat_insens[9]; // insensitive features
		float	*histinsens[9];
