...
/*
filter_responses:
Use each filter( model.filters ) to filter the detection image's feature-pyramid( array of feature cv::Matrices ) respectively.
...
namespace APPLY_RULES3{
	...
	void * thread_each_part( void *p )
	{
		...
		const vector<cv::Mat> &filters = model->x_filters;
		vector<cv::Mat> &SymScore = model->symbols[partSymbol].score;
		...
		// apply deforcv::Mation rule
		vector<cv::Mat> &SymNtScore = model->symbols[partNonTerminal].score;
		vector<cv::Mat> &Ix = model->rules[partNonTerminal][0].Ix;
		vector<cv::Mat> &Iy = model->rules[partNonTerminal][0].Iy;
	...
	void * thread_each_component( void *p )
	{
		...
		vector<cv::Mat> &RootNtScore = model->symbols[rootNonTerminal].score;
		RootNtScore.resize( num_levels );
		vector<cv::Mat> &RootScore = model->symbols[rootSymbol].score;

		for( int lv=interval; lv<num_levels; lv++ ){
			cv::Mat &score = RootNtScore[lv];
			score = RootScore[lv] + cv::Scalar(RuleData.model_bias[lv]);
			int partlv = lv - interval;
			for( int part=0; part<RuleData.numparts; part++ ){
				cv::Mat &pscore = model->symbols[ RuleData.sym_nonTerminal[part+1] ].score[partlv];
...
void	PM_type::apply_rules3( MODEL &model, FEATURE_PYRAMID &pyra )
{
	...
	// calculate filter response and apply deforcv::Mation rules
	...
	for( unsigned component=1; component<model.RuleData.size(); component++ ){		
		...
		for( int lv=model.interval; lv<pyra.num_levels; lv++ ){
			cv::Mat &score1 = model.symbols[rootSym1].score[lv];
			cv::Mat &score2 = model.symbols[rootSym2].score[lv];
	...
	/* method 2. only copy the cv::Mat headers */
